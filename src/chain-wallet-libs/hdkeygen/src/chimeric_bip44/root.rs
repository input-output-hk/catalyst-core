...
use chain_path_derivation::{
    chimeric_bip44::{self, ChimericBip44, CoinType},
    HardDerivation,
};
...
pub struct Root<K> {
    cached_key: Key<K, ChimericBip44<CoinType>>,
}
...
impl<K> Root<K> {
    ...
    pub fn from_cached_key(cached_key: K, derivation_scheme: DerivationScheme) -> Self {
        let cached_key = Key::new_unchecked(
            cached_key,
            chimeric_bip44::new().chimeric().coin_type(COIN_TYPE),
            derivation_scheme,
        );
        Self { cached_key }
    }

    pub(crate) fn cached_key(&self) -> &Key<K, ChimericBip44<CoinType>> {
        &self.cached_key
    }
}
...
impl Root<XPrv> {
    ...
    pub fn from_root_key(root_key: Key<XPrv, ChimericBip44<chimeric_bip44::Root>>) -> Self {
        let path = chimeric_bip44::new().chimeric().coin_type(COIN_TYPE);
    ...
    ) -> Key<XPrv, ChimericBip44<chimeric_bip44::Account>> {
