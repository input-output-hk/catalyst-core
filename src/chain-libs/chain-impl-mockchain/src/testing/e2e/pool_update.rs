...
pub fn pool_update_after_pool_retirement() {
    ...
    assert!(controller
        .retire(&vec![&alice], &stake_pool, &mut ledger)
        .is_ok());
...
pub fn pool_update_wrong_signature_incorrect_owner() {
    ...
    assert!(controller
        .update(&stake_pool, new_stake_pool.clone(), vec![&bob], &mut ledger)
        .is_err());
}
...
pub fn pool_update_changes_owner() {
    ...
    assert!(controller
        .retire(&vec![&bob], &new_stake_pool, &mut ledger)
        .is_ok());
...
pub fn pool_update_revert_changes() {
    ...
    println!(
        "{:?}",
        controller.retire(&vec![&alice], &new_stake_pool, &mut ledger)
    );
