...
pub async fn get_delegator_info(
    ...
    sp_key: serde_json::Value,
    event_id: Option<serde_json::Value>,
) -> Result<crate::models::DelegatorInfo, Error<GetDelegatorInfoError>> {
    ...
    let local_var_uri_str = format!(
        "{}/api/v1/registration/delegations/{sp_key}?event_id={event_id_unwrap}",
    ...
    if let Some(ref local_var_str) = event_id {
        local_var_req_builder =
            local_var_req_builder.query(&[("event_id", &local_var_str.to_string())]);
    }
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<GetDelegatorInfoError> =
...
pub async fn get_voter_info(
    ...
    event_id: Option<serde_json::Value>,
    with_delegators: Option<serde_json::Value>,
) -> Result<crate::models::VotersInfo, Error<GetVoterInfoError>> {
    let local_var_configuration = configuration;
    ...
    if let Some(ref local_var_str) = with_delegators {
        local_var_req_builder =
            local_var_req_builder.query(&[("with_delegators", &local_var_str.to_string())]);
    }
    if let Some(ref local_var_user_agent) = local_var_configuration.user_agent {
        local_var_req_builder =
            local_var_req_builder.header(reqwest::header::USER_AGENT, local_var_user_agent.clone());
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<GetVoterInfoError> =
            serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent {
            status: local_var_status,
            content: local_var_content,
