...
#[test]
pub fn fees_go_to_with_capping_about_threshold() {
    let total_reward = 100;
    let expected_stake_pool_reward = 50;
    ...
    let (mut ledger, controller) = prepare_scenario()
        .with_config(
            ConfigBuilder::new()
                ...
                .with_pool_reward_participation_caping(1, 3)
                .with_rewards_params(RewardParams::Linear {
                    constant: total_reward + 1,
                    ratio: Ratio {
                        numerator: 1,
        ...
        .with_initials(vec![wallet("Alice").with(1_000).owns("stake_pool")])
        .with_stake_pools(vec![stake_pool("stake_pool").tax_ratio(1, 2).tax_limit(60)])
        .build()
        .unwrap();
    ...
    ledger_verifier
        .account(alice.as_account_data())
        .has_value(&Value(1_000 + 50));
}

#[test]
pub fn fees_go_to_with_capping_below_threshold() {
    let total_reward = 100;
    let expected_stake_pool_reward = 50;
    ...
    let (mut ledger, controller) = prepare_scenario()
        .with_config(
            ConfigBuilder::new()
                ...
                .with_treasury(Value::zero())
                .with_fees_in_treasury()
                .with_pool_reward_participation_caping(2, 1)
                .with_rewards_params(RewardParams::Linear {
                    constant: total_reward + 1,
                    ratio: Ratio {
                        numerator: 1,
...
#[test]
pub fn fees_go_to_without_any_stake_pool() {
    let total_reward = 100;
    ...
    ledger_verifier
        ...
        .and()
        .has_remaining_rewards_equals_to(&Value(101));

    ledger_verifier
        .account(alice.as_account_data())
        .has_value(&Value(1_000));
}
