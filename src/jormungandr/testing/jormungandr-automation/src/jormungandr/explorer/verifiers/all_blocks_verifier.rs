...
impl ExplorerVerifier {
                        ...
                        match &explorer_transaction.certificate {
                            None => {
                                if let Fragment::Transaction(fragment_transaction) = fragment {
                                    ...
                                    let explorer_config_param = &explorer_transaction
                                        ...
                                        .unwrap()
                                        .config_params;
                                    Self::assert_all_block_config_param(
                                        config_params,
                                        explorer_config_param,
                                    );
                                } else {
                            ...
                            Some(certificate) => {
                                match certificate{
                                        ...
                                        if !(explorer_block.date.epoch.id.parse::<u32>().unwrap() == 0 &&
                                            explorer_block.date.slot.parse::<u32>().unwrap() == 0){
                                           return Err(VerifierError::InvalidCertificate {
    ...
    fn assert_all_block_config_param(
        ...
        for block0_param in config_params.iter() {
            match block0_param {
                Block0Date(certificate_param) => {
                    let matching_params = explorer_config_param
                        .iter()
                        .filter(|&config_param| {
