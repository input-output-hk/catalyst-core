...
/// struct for typed errors of method [`get_all_proposals`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetAllProposalsError {
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`get_proposal`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
...
pub async fn get_all_proposals(configuration: &configuration::Configuration, id: i32, obj_id: i32, limit: Option<serde_json::Value>, offset: Option<serde_json::Value>, grp: Option<serde_json::Value>) -> Result<serde_json::Value, Error<GetAllProposalsError>> {
    ...
    let local_var_uri_str = format!("{}/api/v1/event/{id}/objective/{obj_id}/proposals", local_var_configuration.base_path);
    ...
    if let Some(ref local_var_str) = grp {
        local_var_req_builder = local_var_req_builder.query(&[("grp", &local_var_str.to_string())]);
    }
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<GetAllProposalsError> = serde_json::from_str(&local_var_content).ok();
...
pub async fn get_proposal(configuration: &configuration::Configuration, id: serde_json::Value, obj_id: serde_json::Value, prop_id: serde_json::Value) -> Result<serde_json::Value, Error<GetProposalError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/api/v1/event/{id}/objective/{obj_id}/proposal/{prop_id}", local_var_configuration.base_path, id=id, obj_id=obj_id, prop_id=prop_id);
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::GET, local_var_uri_str.as_str());
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<GetProposalError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
