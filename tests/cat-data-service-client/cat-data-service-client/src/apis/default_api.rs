...
/// struct for typed errors of method [`health_live`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum HealthLiveError {
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`health_ready`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum HealthReadyError {
    UnknownValue(serde_json::Value),
}


/// liveness of the service
pub async fn health_live(configuration: &configuration::Configuration, ) -> Result<serde_json::Value, Error<HealthLiveError>> {
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<HealthLiveError> = serde_json::from_str(&local_var_content).ok();
...
pub async fn health_ready(configuration: &configuration::Configuration, ) -> Result<serde_json::Value, Error<HealthReadyError>> {
    let local_var_configuration = configuration;
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<HealthReadyError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
