...
pub fn insert_proposal(proposal: Proposal, db_conn: &DbConnection) -> QueryResult<usize> {
    execute_q!(
        db_conn,
        diesel::insert_into(proposals::table).values(proposal.values())
    )
}
...
pub fn batch_insert_proposals(
    ...
    execute_q!(
        db_conn,
        diesel::insert_into(proposals::table).values(
            proposals_slice
                .iter()
                .cloned()
...
pub fn batch_insert_groups(
    ...
    execute_q!(
        db_conn,
        diesel::insert_into(groups::table).values(gs.map(|g| g.values()).collect::<Vec<_>>())
    )
}
...
pub fn batch_insert_community_choice_challenge_data(
    ...
    execute_q!(
        db_conn,
        diesel::insert_into(community_choice_proposal_dsl::table).values(values)
    )
}
...
pub fn batch_insert_simple_challenge_data(
    ...
    execute_q!(
        db_conn,
        diesel::insert_into(simple_proposal_dsl::table).values(values)
    )
}
