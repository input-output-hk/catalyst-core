...
impl Wallet {
    ...
    pub fn from_mnemonics(mnemonics: String, password: Vec<u8>) -> Result<Self, WalletError> {
        let wallet = InnerWallet::recover(&mnemonics, &password)?;

        Ok(Self(Mutex::new(wallet)))
    }

    pub fn convert(&self, settings: Arc<Settings>, valid_until: BlockDate) -> Conversion {
        let mut wallet = self.0.lock().unwrap();
        let settings = settings.0.lock().unwrap();

        // TODO: change the signature of convert to take a reference to the settings instead
        let conversion = wallet.convert(settings.clone(), &valid_until.into());

        Conversion {
            ignored_value: conversion
                .ignored()
                .iter()
                .map(|input| input.value().0)
                .sum(),
            ignored_count: conversion.ignored().len() as u32,
            fragments: conversion
                .transactions()
                .iter()
                // unwrapping here because fragments generated by the library should deserialize
                // just fine.
                // a refactor would be needed somehow to skip useless
                // serialization/deserialization, but it can be done later.
                .map(|raw| Fragment::new(raw.to_vec()).unwrap())
                .map(Arc::new)
                .collect(),
        }
    }
