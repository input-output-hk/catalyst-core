...
A request not validly made by the client will not include the `anti-csrf-token`.
The attacker therefore can not guess the random key used to calculate the keyed hash of the [Client Auth Token](#client-auth-token).

The server can then validate the `anti-csrf-token` is correct for the [Client](#client-auth-token) Auth Token](#client-auth-token).
