...
COMMENT ON COLUMN voting_nodes.seckey IS 'Secret key from Ed25519 pair for the node. NULL = not yet defined.';
COMMENT ON COLUMN voting_nodes.pubkey IS 'Public key from Ed25519 pair for the node. NULL = not yet defined.';
COMMENT ON COLUMN voting_nodes.netkey IS 'Ed25519 secret key for the node. NULL = not yet defined.';
COMMENT ON COLUMN voting_nodes.extra IS 'Json Map defining node specific extra data, particularly the "leadership" information. NULL = Not yet defined. "leadership"."role" = one of: "leader0", "leader", or "follower", "leadership"."consensus_id" = a Ed25519Extended public key used only by "leader0" or "leader" nodes.';
