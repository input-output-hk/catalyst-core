  ...
  get(target, matcherName, receiver) {
    ...
    return (...args) => {
      ...
      const reportStepError = jestError => {
        ...
        if (!this._info.isSoft) throw error;
      };
      const finalizer = () => {
        step.complete({});
      };
      try {
        const expectZone = matcherName !== 'toPass' ? {
          title,
          wallTime
        } : null;
        const callback = () => matcher.call(target, ...args);
        const result = expectZone ? _utils.zones.run('expectZone', expectZone, callback) : _utils.zones.preserve(callback);
