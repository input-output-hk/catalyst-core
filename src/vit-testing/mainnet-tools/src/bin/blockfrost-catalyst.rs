...
use mainnet_lib::{CatalystBlockFrostApi, CatalystBlockFrostApiError, GeneralTransactionMetadataInfo};
...
async fn main() -> Result<(), CatalystBlockFrostApiError> {
    ...
    registrations.iter().for_each(|reg| println!("{}", reg.to_json_string(MetadataJsonSchema::BasicConversions).unwrap()) );
    Ok(())
}
