        ...
        it('decrypts keys correctly', function (done) {
            symmetricCipherDecrypt(PASSWORD, hexStringToBytes(ENCRYPTED_WALLET))
                .then(function (decryptedKeys) {
                    const account = decryptedKeys.buffer.slice(0 * 64, 1 * 64);
                    const key1 = decryptedKeys.buffer.slice(64, 2 * 64);
                    const key2 = decryptedKeys.buffer.slice(2 * 64, 3 * 64);

                    if (hexStringToBytes(ACCOUNT) !== new Uint8Array(account)) {
                        done.fail('wrong expected account');
                    }
                    if (hexStringToBytes(KEY1) !== new Uint8Array(key1)) {
                        done.fail('wrong expected key1');
                    }
                    if (hexStringToBytes(KEY2) !== new Uint8Array(key2)) {
