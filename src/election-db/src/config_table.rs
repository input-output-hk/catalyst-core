...
fn get_config_records(key : &ConfigKey ) -> 
    Result<(/*TODO*/), Box<dyn Error + Send + Sync + 'static>> {
...
///   * Or there is a DB error of some kind.
fn get_config_record(key : &ConfigKey ) -> 
    Result<Option<serde_json::Value>, Box<dyn Error + Send + Sync + 'static>>  {

    let results = get_config_records(key)?;
...
/// 
fn set_config_record(key : &ConfigKey, value: &serde_json::Value ) -> 
...
impl ElectionDB {
        ...
        Result<Authorization, Box<dyn Error + Send + Sync + 'static>> 
    {
        ...
        let auth = get_config_record(
            &ConfigKey::api_token(encrypted_token))?;
